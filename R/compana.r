...
"compana" <- function(used, avail, test = c("randomisation", "parametric"),
                            rnv = 0.01, nrep = 1000, alpha=0.05)      # Changed the default parameters.
{
  ## 1. Verifications ####
  test<-match.arg(test)
  used<-as.matrix(used)
  ...
  if (test=="randomisation") {
    ...
    for (i in 1:nh) {
      for (j in 1:nh) {
        ...
        ## standard deviations (root mean square error)
        rmse[i,j]<-sqrt(var(dlr)/na)                                   
        
# ----- ## 95% confidence interval
        rmci[i,j]<-1.96*rmse[i,j]                                       
        
        ## t-test statistic
        if (i!=j)
          rmv[i,j]<-rdlr[i,j]/rmse[i,j]
  ...
  rownames(rmse)<-colnames(avail) 
  colnames(rmse)<-colnames(avail) 
  rownames(rmci)<-colnames(avail) 
  colnames(rmci)<-colnames(avail) 
  rownames(rdlr)<-colnames(avail) 
  colnames(rdlr)<-colnames(avail) 
